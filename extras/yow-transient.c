 ...
 */
#ifndef lint
static char sccs_id[] = "@(#)yow.c	1.6 12/18/87  Copyright (c) 1987, Siemens Corporate Research and Support, Inc.";
#endif
...
Window     client_icon;   /* The client icon window */
Window     transient_window; /* The transient window */
...
static int use_client_icon = 0;
static int use_transient = 0;
static int transient_mapped = 0;
static int transient_size = 150;
    ...
    char *argv[];
{
    ...
    while ((c = getopt(argc, argv, "r:w:i:c:b:e:s:p:f:g:d:ut")) != EOF)
    {
	switch (c)
	{
	case 'r':
...
/* ------------------------------------------------------------ */
    ...
    /* Create the client icon that the application will use
     *   as a subwindow of that parent window.
     */

void RTLib_Create_Client_Icon()
{
    unsigned long  border, background;
    unsigned int  border_width_val;
    ...
    client_icon = XCreateSimpleWindow(dpy, parent_window,
				      desired_x, desired_y,
				      desired_width, desired_height,
				      border_width_val,
				      border,
...
void RTLib_Create_Transient_Window()
{
    unsigned long  border, background;
    unsigned int  border_width_val;
    unsigned int desired_x, desired_y;
    ...
    desired_x = transient_size;
    desired_y = transient_size;
    desired_width = transient_size;
    desired_height = 64;
    border_width_val = 6;

    transient_window = XCreateSimpleWindow(dpy, parent_window,
				      desired_x, desired_y,
				      desired_width, desired_height,
				      border_width_val,
				      border,
    ...
    XSetTransientForHint(dpy, transient_window, window);
    XFlush(dpy);
}    
/* ------------------------------------------------------------ */
...
void RTLib_Set_WM_Hints()
{
	...
	wmhints.icon_window = client_icon;
    }
...
{
    if (xbutton.state & (ControlMask | ShiftMask))
        exit(0);
	...
	if(use_client_icon)
	{
	    XMapWindow(dpy, client_icon);
	    XClearWindow(dpy, client_icon);
	    XDrawString(dpy, client_icon, gc, 1, 32, "ICON",
			strlen("ICON"));
	    XFlush(dpy);
	}
        return;
    ...
    {
	if(use_transient && (transient_mapped == 0))
	{
	    transient_mapped = 1;
	    XMapWindow(dpy, transient_window);
	    XFlush(dpy);
	    sleep(1);
	    XClearWindow(dpy, transient_window);
	    XDrawString(dpy, transient_window, gc, 1, 32,
			"TRANSIENT", strlen("TRANSIENT"));
	    XFlush(dpy);
	}
    }
    else if (xbutton.button == Button2)
    {
	if(use_transient && (transient_mapped == 1))
	{
	    transient_mapped = 0;
	    XUnmapWindow(dpy, transient_window);
	}
    }
	...
	if(use_transient && (transient_mapped == 1))
	{
	    if (transient_size == 150)
		transient_size = 300;
	    else
		transient_size = 150;

	    XMoveResizeWindow(dpy, transient_window,
			      transient_size, 
			      transient_size, 
			      transient_size, 
			      64);
    ...
    char *argv[];
{
    ...
    RTLib_Create_Window();
    if (use_client_icon)
	RTLib_Create_Client_Icon();
    if (use_transient)
	RTLib_Create_Transient_Window();
    ...
    printf("Type <CR> to map the window!\n");
    scanf("%c", &chr);
